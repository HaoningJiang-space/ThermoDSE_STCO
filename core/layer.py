...
class Layer():
    ...
    def get_hifm(self):
        return self.input_layer().hofm  # Height of IFM

    def get_wifm(self):
        return self.input_layer().wofm  # Weight of IFM
...
class groupConvLayer(ConvLayer):
    '''
    a special case of ConvLayer,
    
    multiple convolution layers and concate as OFM
    '''
    def __init__(self, nifm, nofm, sofm, sfil, numG, concat_hwc, strd=1):
        if isinstance(sofm, int):
            hofm = sofm
            wofm = sofm
        elif len(sofm) == 2:
        ...
        if concat_hwc[2]: 
            ...
            raise ValueError(f'For GroupConvLayer, the OFMs of all layers must be concated')
        super(groupConvLayer, self).__init__(nifm, nofm, (hofm, wofm), sfil, strd)
        self.numG = numG
...
class DPConvLayer(ConvLayer):
    '''
    Depthwise convolution layer 
    
    As a special case of conv layer

    '''
    def __init__(self, nofm, sofm, sfil, strd=1):
        super(DPConvLayer, self).__init__(nofm, nofm, sofm,sfil,strd)

    def ops_per_neuron(self):
        # DP conv only for 1 channel.
    ...
    def __repr__(self):
        return '{}({})'.format(
            ...
            ', '.join([
                ...
                'sofm={}'.format(repr(self.hofm, self.wofm)),
                'sfil={}'.format(repr((self.hfil, self.wfil))),
                'strid={}'.format(repr(self.htrd, self.wtrd)),
                ])),
                
    
class GemmLayer(Layer):
    '''
    GEMM layer for transformer A(M x K) x B(K x M) = C(M x N)
    M = hofm * wofm
    N = nofm
    K = nifm
    '''

    def __init__(self, nifm, nofm, sofm, strd=1, mtxBT=False):
        super(GemmLayer, self).__init__(nofm, sofm, strd=strd)
        if sofm[1] == 1:
            self.smtxA = (sofm[0],sofm[1], nifm)
        else:
            raise ValueError('Gemm: nofm[1] is must be 1 (it is {}),'.format(nofm[1]))
        self.smtxB = (nifm, 1 , nofm)
        self.mtxBT = mtxBT
        self.inlayer = Layer(nifm, (sofm[0], nofm))
        self.hfil = 1
        self.wfil = 1
    ...
    def total_mtxB_size(self, word_bytes):
        return self.smtxB[0] * self.smtxB[1] * self.smtxB[2] * word_bytes
    ...
    def __repr__(self):
        return '{}({})'.format(
            ...
            ', '.join([
                'nofm={}'.format(repr(self.nofm)),
                'sofm={}'.format(repr((self.hofm, self.wofm))),
                'smtxA={}'.format(repr((self.smtxA[0], self.smtxA[1], self.smtxA[2]))),
                'smtxB={}'.format(repr((self.smtxB[0], self.smtxB[1], self.smtxA[2])))]))
...
class LocalRegionLayer(Layer):
    ...
    def __init__(self, nofm, sofm, nreg, sreg, ntrd=1, strd=1, up=0):
        ...
        nifm = self.nofm * self.ntrd if up == 0 else self.nofm // self.ntrd  # ignore all-zero padding channels.
        # this setting includes the zero mapping of IFM,
        ...
        hifm = self.hofm  * self.htrd if up == 0 else self.hofm // self.htrd 
        wifm = self.wofm  * self.wtrd if up == 0 else self.wofm // self.wtrd
    ...
    def total_ifmap_size(self, batch_size=1, word_size=1):
        return super().total_ifmap_size(batch_size, word_size) * self.nreg
...
class UpsampleLayer(LocalRegionLayer):
    '''
    NN upsample layer parameters.

    As a special case of LocalRegionLayer.

    nreg = ntrd = 1
    '''
    def __init__(self, nofm, sofm, scale):
        super(UpsampleLayer, self).__init__(nofm, sofm, nreg=1, sreg = scale,
                                           ntrd=1, strd=scale, up=1)
...
# class PostProLayer(LocalRegionLayer):
    '''
    post processing layer includes  
