...
# NN.add('conv1', ConvLayer(7*7*3, 64, 112, 1, 1))
...
for i in range(3):
    ...
    if i == 0:
        NN.add('conv2_br', ConvLayer(64, 256, 56, 1), ifm_prevs=(RES_PREV,))
        RES_PREV = 'conv2_br'
    NN.add('conv2_{}_res'.format(i), EltwiseLayer(256, 56, 2),
           ifm_prevs=(RES_PREV, 'conv2_{}_c'.format(i)))
...
for i in range(4):
    ...
    # With residual shortcut.
    if i == 0:
        NN.add('conv3_br', ConvLayer(256, 512, 28, 1, 2), ifm_prevs=(RES_PREV,))
        RES_PREV = 'conv3_br'
    NN.add('conv3_{}_res'.format(i), EltwiseLayer(512, 28, 2),
           ifm_prevs=(RES_PREV, 'conv3_{}_c'.format(i)))
...
for i in range(6):
    ...
    # With residual shortcut.
    if i == 0:
        NN.add('conv4_br', ConvLayer(512, 1024, 14, 1, 2), ifm_prevs=(RES_PREV,))
        RES_PREV = 'conv4_br'
    NN.add('conv4_{}_res'.format(i), EltwiseLayer(1024, 14, 2),
           ifm_prevs=(RES_PREV, 'conv4_{}_c'.format(i)))
...
for i in range(3):
    ...
    # With residual shortcut.
    if i == 0:
        NN.add('conv5_br', ConvLayer(1024, 2048, 7, 1, 2), ifm_prevs=(RES_PREV,))
        RES_PREV = 'conv5_br'
    NN.add('conv5_{}_res'.format(i), EltwiseLayer(2048, 7, 2),
           ifm_prevs=(RES_PREV, 'conv5_{}_c'.format(i)))
