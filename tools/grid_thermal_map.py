...
with open(flp_filename, "r") as fp:
  for line in fp:
    ...
    y = float(parts[4])
    if 'io' in name:
        rectangle = plt.Rectangle((x, y), width, length, fc="none", ec="black", alpha=0.5)
...
with open(temperatures_filename, "r") as fp:
  for line in fp:
    temps.append(float(line.strip().split()[1]) - 273)

temps = np.reshape(temps, (rows, cols))

colors = ['green', 'yellow', 'red']
custom_cmap = LinearSegmentedColormap.from_list('my_hot', colors, N=256)

# if min_temp is None and max_temp is None:
#   norm = mcolors.PowerNorm(gamma=0.3, vmin =np.min(temps), vmax=np.max(temps))
# else:
#   norm = mcolors.PowerNorm(gamma=0.3, vmin =min_temp, vmax=max_temp)

im = axs.imshow(temps, cmap= 'turbo', extent=(0, total_width, 0, total_length), origin='lower')
...
cbar = fig.colorbar(im, ax=axs, pad= 0.01, fraction=0.03)

cbar.ax.tick_params(labelsize=16)   # ← 控制色条刻度字体

axs.set_title(f"Max. Temp.= {round(np.max(temps),1)}℃", fontsize=20)
