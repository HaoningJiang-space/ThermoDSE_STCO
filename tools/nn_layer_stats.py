...
def layer_stats(args):
    ...
    for name in network:
        ...
        layer = network[name]
        # print(f'{name} previous layer: {network.prevs(name)}, sifm:({layer.hifm, layer.wifm, layer.get_nifm()}). sofm:({layer.hofm, layer.wofm, layer.nofm}), stride:{layer.htrd, layer.wtrd}')
        ...
        min_opt_buf_size = min(
            ...
            filter_size + (ifmap_size / layer.get_nifm() + ofmap_size) / batch,
            ifmap_size + (ofmap_size + filter_size / layer.get_nifm()) / layer.nofm,
            ifmap_size + (ofmap_size / batch + filter_size) / layer.nofm,
            ofmap_size + (ifmap_size + filter_size / layer.nofm) / layer.get_nifm(),
            ofmap_size + (ifmap_size / batch + filter_size) / layer.get_nifm())

        sys.stdout.write(line_fmt
                         .format(name,
                                 ifmap_size, ofmap_size, filter_size,
                                 ops, min_opt_buf_size))
        max_ifmaps = max(max_ifmaps, ifmap_size)
    ...
    sys.stdout.write(line_fmt
                     .format('MAX',
                             max_ifmaps, max_fmaps, max_filters,
                             max_ops, float('nan')))
    sys.stdout.write(line_fmt
                     .format('SUM',
